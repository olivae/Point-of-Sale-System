...
namespace MobileApp
{
	...
	public partial class salExtras : PopupPage
	{
        ...
        public salExtras (int cusName, string itemName, string oList, decimal oTotal, List<ProductInfo> cart)
		{
			InitializeComponent ();
        ...
        void onContinue(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    if (size.ToLower().Contains("small")) { iTotal += 3.00m; item += " Small"; }
                    else if (size.ToLower().Contains("medium")) { iTotal += 5.00m; item += " Medium"; }
                    else if (size.ToLower().Contains("large")) { iTotal += 7.00m; item += " Large"; }
                }

             

                if (size == null) { DisplayAlert("Error!", "Please specify a size for your order", "OK"); }
                else
                {
